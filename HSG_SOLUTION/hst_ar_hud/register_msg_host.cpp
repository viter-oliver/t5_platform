...
void register_msg_host(msg_utility::msg_host &mh) {
  ...
  mh.register_msg_handle(lane_frame_1_l, [&](u8 *pbuff, int len) {
	...
	int delta=chrono::duration_cast<chrono::duration<int, std::milli>>( nw_tm - tp_lc0).count();
	static int dcnt = 0;
    if (dcnt == 0) {
    ...
    dcnt%=10;
    tp_lc0=nw_tm;
    hud::lc0 = pcan->Position * 0.00390625; //-128;
    hud::lc2 = pcan->Curvature * 0.000000976563 - 0.031999023438;
    hud::lc3 = pcan->CurvatureDerivative * 0.00000000372529 -0.00012206658721;
  ...
  mh.register_msg_handle(lane_frame_2_l, [&](u8 *pbuff, int len) {
    ...
    hud::rc0 = pcan->Position * 0.00390625; //-128;
    hud::rc2 = pcan->Curvature * 0.000000976563 - 0.031999023438;
    hud::rc3 = pcan->CurvatureDerivative * 0.00000000372529 -0.00012206658721;
  ...
  //TODO: ivi navigation
  #ifdef IVI_NAVI
  mh.register_msg_handle(HUDSERVICE_NEXT_INTERSECTION_NAME_INFO,[&](u8 *pbuff, int len){
    return true;
  });
  mh.register_msg_handle(HUDSERVICE_NEXT_ICON_INFO,[&](u8 *pbuff, int len){
    return true;
  });
  mh.register_msg_handle(HUDSERVICE_NEXT_CROSS_DISTANCE_INFO,[&](u8 *pbuff, int len){
    return true;
  });
  mh.register_msg_handle(HUDSERVICE_TARGET_DISTANCE_INFO,[&](u8 *pbuff, int len){
    return true;
  });
  #endif
}