    ...
    if (stm._play_state == en_play_tran) {
      ...
      if (delta_tm > 0) {
        ...
        for (auto &prop : prop_list) {
          ...
          } else if (fel._type == "af_vec2") {
            af_vec2 *pfrom = (af_vec2 *)&ppt_blk_from[0];
            af_vec2 *pto = (af_vec2 *)&ppt_blk_to[0];
            af_vec2 *pvl = (af_vec2 *)ppt_addr;
            for (int ix = 0; ix < fel._count; ix++) {
          ...
          } else if (fel._type == "af_vec3") {
            af_vec3 *pfrom = (af_vec3 *)&ppt_blk_from[0];
            af_vec3 *pto = (af_vec3 *)&ppt_blk_to[0];
            af_vec3 *pvl = (af_vec3 *)ppt_addr;
            for (int ix = 0; ix < fel._count; ix++) {
              step_value(&pfrom->x, &pto->x, &pvl->x, value_scale);
              step_value(&pfrom->y, &pto->y, &pvl->y, value_scale);
          ...
          } else if (fel._type == "af_vec4") {
            af_vec4 *pfrom = (af_vec4 *)&ppt_blk_from[0];
            af_vec4 *pto = (af_vec4 *)&ppt_blk_to[0];
            af_vec4 *pvl = (af_vec4 *)ppt_addr;
            for (int ix = 0; ix < fel._count; ix++) {
              step_value(&pfrom->x, &pto->x, &pvl->x, value_scale);
              step_value(&pfrom->y, &pto->y, &pvl->y, value_scale);
              step_value(&pfrom->z, &pto->z, &pvl->z, value_scale);
      ...
      const auto &itrans = stm._mtrans.find(cur_trankey);
