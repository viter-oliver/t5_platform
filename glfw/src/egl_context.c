                                ...
                                EGLConfig* result)
{
    ...
    usableCount = 0;
    printf("%s:nativeCount=%d\n",__FUNCTION__,nativeCount);
    for (i = 0;  i < nativeCount;  i++)
    {
        ...
        int icong_value=getEGLConfigAttrib(n, EGL_COLOR_BUFFER_TYPE);
        if (icong_value != EGL_RGB_BUFFER)
        {
            continue;
        }

        // Only consider window EGLConfigs
        icong_value=getEGLConfigAttrib(n, EGL_SURFACE_TYPE);
        if (!(icong_value & EGL_WINDOW_BIT))
        {
            continue;
        }
        ...
        {
            icong_value=getEGLConfigAttrib(n, EGL_RENDERABLE_TYPE);
            if (ctxconfig->major == 1)
            {
                if (!(icong_value & EGL_OPENGL_ES_BIT))
                    continue;
            }
            else
            {
                if (!(icong_value & EGL_OPENGL_ES2_BIT))
                    continue;
            }
        }
        ...
        {
            icong_value=getEGLConfigAttrib(n, EGL_RENDERABLE_TYPE);
            printf("%d:icong_value=%d\n",__LINE__,icong_value);
            if (!( icong_value & EGL_OPENGL_BIT))
                continue;
        }
...
static void makeContextCurrentEGL(_GLFWwindow* window)
{
    if (window)
    {
                            ...
                            window->context.egl.handle))
        {
         ...
         printf("_glfw.egl.display=%d, window->context.egl.surface=%d,window->context.egl.handle=%d\n",\
...
static void swapBuffersEGL(_GLFWwindow* window)
{
   ...
   // printf("_glfw.egl.display=%d,window->context.egl.surface=%d\n",_glfw.egl.display,window->context.egl.surface);
}
...
int choose_format(EGLDisplay egl_disp, EGLConfig egl_conf)
{
    ...
    printf("buffer_bit_depth=%d,alpha_bit_depth=%d\n",buffer_bit_depth,alpha_bit_depth);
                               ...
                               const _GLFWfbconfig* fbconfig)
{
    ...
    config=choose_config(_glfw.egl.display,"rgba8888");
    ...
    int format=choose_format( _glfw.egl.display, config);
    printf("format=%d\n",format);
